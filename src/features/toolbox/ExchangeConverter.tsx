  ...
  return (
    <Card className="p-5 lg:p-7">
      ...
      <div className="mt-7 grid items-end gap-4 md:grid-cols-[1fr_1fr_auto_1fr]">
        <div>
          <label htmlFor="exchange-amount" className="text-xs font-medium text-slate-600">金额</label>
          <Input id="exchange-amount" name="exchange-amount" className="mt-1.5" type="number" inputMode="decimal" value={amount} onChange={(event) => setAmount(Number(event.target.value))} />
        </div>
        <div>
          <label htmlFor="exchange-from" className="text-xs font-medium text-slate-600">从</label>
          <select id="exchange-from" name="exchange-from" className={`${selectClass} mt-1.5`} value={from} onChange={(event) => setFrom(event.target.value)}>{currencies.map((currency) => <option key={currency.code} value={currency.code}>{currency.name}（{currency.code}）</option>)}</select>
        </div>
        <Button type="button" variant="outline" aria-label="交换币种" onClick={() => { setFrom(to); setTo(from) }}><ArrowRightLeft className="h-4 w-4" /></Button>
        <div>
          <label htmlFor="exchange-to" className="text-xs font-medium text-slate-600">换算为</label>
          <select id="exchange-to" name="exchange-to" className={`${selectClass} mt-1.5`} value={to} onChange={(event) => setTo(event.target.value)}>{currencies.map((currency) => <option key={currency.code} value={currency.code}>{currency.name}（{currency.code}）</option>)}</select>
        </div>
      </div>
