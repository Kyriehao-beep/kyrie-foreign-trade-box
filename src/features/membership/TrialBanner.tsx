  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-3 border-b border-brand-100 bg-brand-50 px-5 py-2.5 text-sm text-brand-900 print:hidden">
      <span className="flex items-center gap-2 font-medium">
        ...
        {error || (status.phase === '试用中' ? `完整试用中，还可使用约 ${status.remainingDays} 天` : status.phase === '已激活' ? '会员功能已激活' : status.phase === '已停用' ? '账号已停用，请联系人工客服' : status.phase === '未登录' ? '登录后可开启 3 天完整试用' : '免费试用已结束')}
      </span>
