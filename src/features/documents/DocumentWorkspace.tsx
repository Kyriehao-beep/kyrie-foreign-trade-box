...
import { clearDraft, loadDraft, saveDraft, savePartyTemplate } from '../../services/storage'
...
export function DocumentWorkspace() {
  ...
  function saveTemplate(kind: 'seller' | 'buyer') {
    ...
    setSaveStatus(result.ok ? '资料模板已保存' : result.error)
  }
      ...
      <div className="grid min-w-0 gap-6 lg:grid-cols-[minmax(0,1.05fr)_minmax(520px,.95fr)]">
        <div className={`${mobileView === 'preview' ? 'hidden lg:block' : ''} min-w-0 space-y-4 print:hidden`}>
          ...
          {(['seller', 'buyer'] as const).map((kind) => {
            ...
            return <Card className="p-5" key={kind}><SectionTitle action={<Button type="button" size="sm" variant="ghost" onClick={() => saveTemplate(kind)}><Save className="h-3.5 w-3.5" />保存为模板</Button>}>{prefix}资料</SectionTitle><div className="grid gap-4 sm:grid-cols-2"><Field label={`${prefix}公司名称`} value={party.companyName} review={draft.reviewFields.includes(`${prefix}公司名称`)} onChange={(value) => updateParty(kind, 'companyName', value)} /><Field label={`${prefix}国家或地区`} value={party.country} onChange={(value) => updateParty(kind, 'country', value)} /><Field label={`${prefix}地址`} value={party.address} review={draft.reviewFields.includes(`${prefix}地址`)} onChange={(value) => updateParty(kind, 'address', value)} /><Field label={`${prefix}联系人`} value={party.contact} onChange={(value) => updateParty(kind, 'contact', value)} /><Field label={`${prefix}电话`} value={party.phone} onChange={(value) => updateParty(kind, 'phone', value)} /><Field label={`${prefix}邮箱`} value={party.email} onChange={(value) => updateParty(kind, 'email', value)} /><Field label={`${prefix}税号`} value={party.taxId} onChange={(value) => updateParty(kind, 'taxId', value)} /></div></Card>
          })}
