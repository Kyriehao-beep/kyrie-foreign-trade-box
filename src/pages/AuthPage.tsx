  ...
  async function submit(event: FormEvent) {
    ...
    try {
      if (mode === 'register') {
        ...
        setMessage(`注册成功，${PRICING_SUMMARY.trialDays} 天完整试用已开启`)
      } else {
  ...
  return (
    <main className="mx-auto grid max-w-6xl gap-8 px-5 py-12 lg:grid-cols-[1fr_480px] lg:items-center lg:px-8 lg:py-20">
      <section>
        ...
        <h1 className="mt-5 text-4xl font-semibold tracking-tight sm:text-5xl">{mode === 'register' ? `创建账号，${PRICING_SUMMARY.trial}` : '登录 Kyrie的外贸盒子'}</h1>
